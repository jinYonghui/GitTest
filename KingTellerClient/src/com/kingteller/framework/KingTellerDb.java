package com.kingteller.framework;
...
import com.kingteller.framework.db.sqlite.CursorUtils;
import com.kingteller.framework.db.sqlite.DbModel;
import com.kingteller.framework.db.sqlite.ManyToOneLazyLoader;
import com.kingteller.framework.db.sqlite.OneToManyLazyLoader;
import com.kingteller.framework.db.sqlite.SqlBuilder;
import com.kingteller.framework.db.sqlite.SqlInfo;
import com.kingteller.framework.db.table.KeyValue;
import com.kingteller.framework.db.table.ManyToOne;
import com.kingteller.framework.db.table.OneToMany;
import com.kingteller.framework.db.table.TableInfo;
import com.kingteller.framework.exception.DbException;
 ...
 *
 */
public class KingTellerDb {

	private static final String TAG = "KingTellerDb";

	private static HashMap<String, KingTellerDb> daoMap = new HashMap<String, KingTellerDb>();

	private SQLiteDatabase db;
	private DaoConfig config;

	private KingTellerDb(DaoConfig config) {
		if (config == null)
	...
	private synchronized static KingTellerDb getInstance(DaoConfig daoConfig) {
		KingTellerDb dao = daoMap.get(daoConfig.getDbName());
		if (dao == null) {
			dao = new KingTellerDb(daoConfig);
	 ...
	 */
	public static KingTellerDb create(Context context, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 *            数据库名称
	 */
	public static KingTellerDb create(Context context, String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 */
	public static KingTellerDb create(Context context, String dbName, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
		config.setDebug(isDebug);
	 ...
	 *            数据库名称
	 */
	public static KingTellerDb create(Context context, String targetDirectory,
			String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
	 ...
	 * @return
	 */
	public static KingTellerDb create(DaoConfig daoConfig) {
