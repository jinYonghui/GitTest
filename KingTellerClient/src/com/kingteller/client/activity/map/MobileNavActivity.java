...
public class MobileNavActivity extends BaseMapActivity implements
	...
	private void initUI() {
		...
		AddressBean address = KingTellerApplication.getApplication()
				.getCurAddress();
		// 移动到当前位置
	...
	public void onClick(View view) {
		...
		case R.id.button_right:
			if (layout_map.getVisibility() == View.VISIBLE) {
				...
				if (start != null) {
					edittext_start.setText(KingTellerApplication
							.getApplication().getCurAddress().getName());
					getNavLine();
				} else
					toastMsg("未获取到您的定位数据");
			}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		switch (requestCode) {
		case CommonCodeConfig.REQUEST_ONATMCODE:
		case CommonCodeConfig.REQUEST_ONCAR:
			...
			break;
		case CommonCodeConfig.REQUEST_OFFCAR:
		case CommonCodeConfig.REQUEST_OFFATMCODE:
			if (Activity.RESULT_OK == resultCode) {
				AddressBean address = (AddressBean) data
						.getSerializableExtra(SelectAddressActivity.ADDRESS);
