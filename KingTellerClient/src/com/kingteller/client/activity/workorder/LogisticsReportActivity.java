 ...
 * 
 */
public class LogisticsReportActivity extends BaseReportActivity{
	...
	private List<PickInfoBean> pickInfoList; // 物流人员
	private List<CostInfoBean> costInfoList; // 费用类型
	...
	public void onCreate(Bundle paramBundle) {
		...
		setContentView(R.layout.layout_logistics_report);
		initUI();
		initData();
		initChangeUI();
	}
	...
	public void initUI() {
		...
		if(CommonUtils.isEmpty(isCheckout)){	
			switch (optType) {
			case OPT_ADUIT:
				title_title.setText("物流报告审核");
				break;
			default:
				title_title.setText("物流报告填写");
				break;
			}
		}else{
			title_title.setText("物流报告");
		}
	}
	...
	private void setDataInfo(LogisticsReportBean data) {
		...
		for (int i = 0; i < data.getEpList().size(); i++) {
			...
			if (i == 0)
				fview = new FeeRPGroupView(LogisticsReportActivity.this, false);
			else
				fview = new FeeRPGroupView(LogisticsReportActivity.this, true);
	...
	public String getFromData() {
		...
		for (int i = 0; i < fcost.size(); i++) {
			...
			data.setCostFee(Double.valueOf(CommonUtils.isEmpty(fcost.get(i)
