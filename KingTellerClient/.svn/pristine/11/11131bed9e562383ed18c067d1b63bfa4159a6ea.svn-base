...
public class OtherMatterReportCheckoutActivity extends BaseReportActivity {
	...
	public void initUI() {
		...
		remark.setFieldCheckoutEnabled(false);
	}
	...
	private void setDataInfo(OtherMatterReportBean data) {
		...
		for (int i = 0; i < data.getEpList().size(); i++) {
			FeeGroupView fview;
			if (i == 0)
				fview = new FeeGroupView(OtherMatterReportCheckoutActivity.this, false);
			else
				fview = new FeeGroupView(OtherMatterReportCheckoutActivity.this, true);
